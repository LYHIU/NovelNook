...
function shelf(){
 shell('<div class="shelf-tools"><label class="search-field">'+ArchiveUI.icon('search')+'<input id="search" type="search" aria-label="搜索我的书架" placeholder="搜索书架" value="'+escapeHtml(query)+'"><button type="button" id="clearShelfSearch" class="icon-button" aria-label="清空书架搜索">'+ArchiveUI.icon('close')+'</button></label><button class="primary" id="addBook">'+ArchiveUI.icon('search')+'找书</button></div><div class="filter-chips" role="group" aria-label="筛选阅读状态">'+chips(['',...STATES],filter,'data-filter')+'</div><div class="sort-row"><span>排序</span><div class="sort-options" role="group" aria-label="书架排序">'+[['updated','最近记录'],['title','书名'],['words','字数'],['status','阅读状态']].map(([value,label])=>'<button data-sort="'+value+'" aria-pressed="'+(sortOrder===value)+'">'+label+'</button>').join('')+'</div></div><div id="list"></div>');
...
function list(){
 ...
 $('#list').innerHTML='<p class="shelf-count" role="status">'+(query.trim()?'找到 ':'共 ')+'<strong>'+visible.length+'</strong> 本书</p>'+visible.map(b=>'<button class="book" data-book="'+escapeHtml(b.id)+'">'+cover(b)+'<span class="book-body"><span class="book-line"><strong>'+ArchiveUI.highlight(b.title,query)+'</strong></span><span class="book-meta"><span>'+ArchiveUI.highlight(b.author||'作者未填',query)+'</span><span class="platform-label">'+escapeHtml(b.platform)+'</span><span class="shelf-words">'+escapeHtml(count(b,true))+'</span></span><span class="status state-'+ArchiveUI.stateCode(b.status)+'">'+escapeHtml(b.status)+'</span><span class="excerpt">'+ArchiveUI.highlight(ArchiveUI.excerpt(b,query),query)+'</span></span><span class="book-arrow" aria-hidden="true">›</span></button>').join('')+(!visible.length?'<div class="empty">'+ArchiveUI.icon('book')+'<h2>'+(books.length?'没有匹配的书籍':'暂无书籍')+'</h2>'+(!books.length?'<button id="firstBook" class="primary">找第一本书</button>':'')+'</div>':'');
...
function detail(id){
 ...
 shell('<div class="detail-toolbar"><button class="back" id="back">'+ArchiveUI.icon('back')+'书架</button><button class="text-button" id="refreshBook">'+ArchiveUI.icon('refresh')+'刷新资料</button></div>'+bookHeading(b)+'<details class="synopsis"><summary>作品文案</summary><p class="prose">'+escapeHtml(b.summary||'暂无文案')+'</p></details><section class="reading-section"><div class="section-head"><div class="reading-current"><label for="state">阅读状态</label><select id="state" class="state-'+ArchiveUI.stateCode(b.status)+'">'+STATES.map(s=>'<option value="'+escapeHtml(s)+'" '+(b.status===s?'selected':'')+'>'+escapeHtml(s)+'</option>').join('')+'</select></div><button id="write" class="primary">'+ArchiveUI.icon('pen')+'记一笔</button></div></section><section class="overview"><div class="section-head"><h2>阅读总览 <small class="tag">'+(b.overview?(b.overviewRevision===b.revision?'':'待更新'):'')+'</small></h2><div><button class="text-button" id="editOverview">编辑</button><button class="text-button" id="ai">'+ArchiveUI.icon('spark')+'AI 整理</button></div></div><p class="prose" id="overview">'+escapeHtml(b.overview||'暂无总览')+'</p></section><section class="notes-section"><div class="section-head"><h2>阅读笔记 <small>'+b.entries.length+' 条</small></h2></div>'+b.entries.slice().reverse().map(n=>'<article class="note"><div class="section-head"><time>'+date(n.createdAt)+(n.progress?' <span class="note-progress">'+escapeHtml(n.progress)+'</span>':'')+'</time><button class="text-button" data-edit="'+escapeHtml(n.id)+'">编辑</button></div><p class="prose">'+escapeHtml(n.text)+'</p></article>').join('')+(!b.entries.length?'<p class="muted">暂无笔记</p>':'')+'</section><button class="text-button delete-book" id="deleteBook">删除这本书</button>');
 ...
 $('#back').onclick=shelf;$('#state').onchange=e=>{const status=e.target.value;if(b.status===status)return;if(change(()=>{b.status=status;b.revision++;b.updatedAt=Date.now();})){detail(id);$('#state').focus({preventScroll:true});}else $('#state').value=books.find(item=>item.id===id).status;};
...
function add(){
 ...
 shell('<button class="back" id="back">'+ArchiveUI.icon('back')+'书架</button><form id="lookup"><label class="field-label">选择平台</label><div class="platform-options" role="group" aria-label="搜索平台">'+[['jinjiang','晋江'],['fanqie','番茄'],['changpei','长佩']].map(([value,label])=>'<button type="button" data-platform="'+value+'" aria-pressed="'+(platform===value)+'"><span class="platform-symbol">'+label.slice(0,1)+'</span><span><strong>'+label+'</strong></span></button>').join('')+'</div><div class="mode-options" role="group" aria-label="搜索方式"><button type="button" data-mode="book" aria-pressed="'+!authorMode+'">书名 / 链接</button><button type="button" data-mode="author" aria-pressed="'+authorMode+'">作者</button></div><label class="field-label" for="keyword">'+(authorMode?'作者名或作者主页':'书名、作品链接或 ID')+'</label><div class="search-line"><label class="search-field">'+ArchiveUI.icon('search')+'<input id="keyword" aria-label="搜索关键词" required maxlength="200" placeholder="'+(authorMode?'输入作者名':'输入书名或粘贴链接')+'" value="'+escapeHtml(lookup.keyword)+'"><button type="button" id="clearKeyword" class="icon-button" aria-label="清空搜索关键词">'+ArchiveUI.icon('close')+'</button></label><button id="lookupSubmit" class="primary" type="submit">搜索</button></div>'+(!authorMode?'<details class="author-option" '+(lookup.author?'open':'')+'><summary>按作者筛选</summary><input id="author" aria-label="作者筛选" placeholder="作者名（可选）" value="'+escapeHtml(lookup.author)+'"></details>':'')+'</form><div id="results" aria-live="polite"></div><button id="screenshotImport" class="import-card">'+ArchiveUI.icon('scan')+'<span><strong>截图识别</strong></span>'+ArchiveUI.icon('arrow')+'</button><div class="manual-link"><button class="text-button" id="manual">手动录入书籍</button></div>');
...
async function inspectBook(id){
 ...
 target.innerHTML=bookHeading(b)+'<div class="import-action"><button class="primary" id="import">'+(existing?'更新书籍资料':'加入书架')+'</button></div><section class="synopsis"><h2>文案</h2><p class="prose">'+escapeHtml(b.summary||'原页未提供可读取文案')+'</p></section>';
 $('#import').onclick=()=>{let selected=existing;
 if(change(()=>{if(existing){ArchiveUI.mergeMetadata(existing,b);}
 else{selected={...b,id:crypto.randomUUID(),status:'想看',entries:[],overview:'',revision:0,overviewRevision:0,updatedAt:Date.now()};books.unshift(selected);}})){lookup={keyword:'',author:'',results:null,mode:'book',authors:[],authorPage:1};detail(selected.id);toast(existing?'资料已更新':'已加入书架');}};
...
function settings(){
 shell('<h1>设置</h1><section class="theme-settings" aria-labelledby="themeLabel"><h2 id="themeLabel">主题</h2><div class="theme-options" role="group" aria-label="主题">'+[['flat','平面绿色'],['glass','液态玻璃']].map(([value,label])=>'<button type="button" class="theme-option" aria-label="'+label+'" data-theme-choice="'+value+'" aria-pressed="'+(ArchiveTheme.current===value)+'"><span class="theme-sample sample-'+value+'" aria-hidden="true"><span class="sample-search"></span><span class="sample-book"></span><span class="sample-lines"></span><span class="sample-nav"></span></span><span class="theme-option-label">'+label+'<span class="theme-check" aria-hidden="true">✓</span></span></button>').join('')+'</div></section><section class="settings-row"><div><h2>本地备份</h2><p class="muted">'+books.length+' 本书</p></div><button id="export">导出</button></section><section class="settings-row"><div><h2>AI 总览</h2><p class="muted">DeepSeek · 点击整理时发送本书笔记</p><p id="aiConfigStatus" role="status" class="config-status">正在检查配置…</p></div></section>');
 document.querySelectorAll('[data-theme-choice]').forEach(el=>el.onclick=()=>{if(!ArchiveTheme.set(el.dataset.themeChoice)){toast('主题保存失败，请重试');return;}document.querySelectorAll('[data-theme-choice]').forEach(button=>button.setAttribute('aria-pressed',button.dataset.themeChoice===ArchiveTheme.current));});
...
[['shelfNav','book','书架'],['addNav','search','找书'],['settingsNav','settings','设置']].forEach(([id,icon,label])=>{$('#'+id).innerHTML=ArchiveUI.icon(icon)+'<span>'+label+'</span>';});
