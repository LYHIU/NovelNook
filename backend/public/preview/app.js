...
function bookHeading(b){return '<div class="book-heading">'+cover(b,true)+'<div><span class="platform-label">'+escapeHtml(b.platform)+'</span><h1>'+escapeHtml(b.title)+'</h1><p class="book-author">'+escapeHtml(b.author||'作者未填')+'</p><p class="book-metrics"><strong>'+escapeHtml(count(b))+'</strong><span>'+escapeHtml(b.serialStatus||'连载状态未提供')+'</span></p>'+sourceLink(b)+'</div></div>';}
